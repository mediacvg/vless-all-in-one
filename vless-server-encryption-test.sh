...
readonly VERSION="3.4.12-test-encryption"
    ...
    case "$base_protocol" in
        vless)
            ...
            jq -n \
                ...
                --arg decryption "$(echo "$cfg" | jq -r '.decryption // .encryption // "none"')" \
...
gen_vless_link() {
    local ip="$1" port="$2" uuid="$3" pbk="$4" sid="$5" sni="$6" country="${7:-}" encryption="${8:-none}"
    local ip_suffix=$(get_ip_suffix "$ip")
    local name="${country:+${country}-}VLESS+Reality${ip_suffix:+-${ip_suffix}}"
    [[ "$encryption" != "none" ]] && name+="-Encryption"
    printf '%s\n' "vless://${uuid}@${ip}:${port}?encryption=${encryption}&security=reality&type=tcp&sni=${sni}&fp=chrome&pbk=${pbk}&sid=${sid}&flow=xtls-rprx-vision#${name}"
}
...
# VLESS+Reality 服务端配置
gen_server_config() {
    local uuid="$1" port="$2" privkey="$3" pubkey="$4" sid="$5" sni="$6" encryption="${7:-none}" decryption="${8:-none}"
    mkdir -p "$CFG"
    
    register_protocol "vless" "$(build_config         uuid "$uuid" port "$port" private_key "$privkey"         public_key "$pubkey" short_id "$sid" sni "$sni" encryption "$encryption" decryption "$decryption")"
    
    _save_join_info "vless" "REALITY|%s|$port|$uuid|$pubkey|$sid|$sni|$encryption"         "gen_vless_link %s $port $uuid $pubkey $sid $sni '' $encryption"
...
show_single_protocol_info() {
    ...
    if [[ -n "$ip_addr" ]]; then
        ...
        case "$protocol" in
            vless)
                local encryption=$(echo "$cfg" | jq -r '.encryption // "none"')
                link=$(gen_vless_link "$ip_addr" "$link_port" "$uuid" "$public_key" "$short_id" "$sni" "$country_code" "$encryption")
                join_code=$(echo "REALITY|${ip_addr}|${link_port}|${uuid}|${public_key}|${short_id}|${sni}|${encryption}" | base64 -w 0)
                ;;
            vless-xhttp)
...
do_install_server() {
    ...
    case "$protocol" in
        vless)
            local reality_profile="classic"
            local encryption="none"
            local decryption="none"
            local enc_mode="xorpub"
            echo ""
            _line
            echo -e "  ${W}VLESS+Reality 传输模式${NC}"
            _line
            _item "1" "标准模式 ${D}(默认, encryption=none)${NC}"
            _item "2" "Encryption 模式 ${D}(实验性 VLESS Encryption)${NC}"
            echo ""
            read -rp "  请选择 [1]: " reality_profile_choice
            reality_profile_choice="${reality_profile_choice:-1}"
            case "$reality_profile_choice" in
                1) reality_profile="classic" ;;
                2)
                    reality_profile="encryption"
                    echo ""
                    echo -e "  ${W}Encryption 混淆方式${NC}"
                    _item "1" "xorpub ${D}(推荐，兼容和混淆折中)${NC}"
                    _item "2" "native ${D}(原始格式)${NC}"
                    _item "3" "random ${D}(随机化更强)${NC}"
                    echo ""
                    read -rp "  请选择 [1]: " vless_enc_choice
                    vless_enc_choice="${vless_enc_choice:-1}"
                    case "$vless_enc_choice" in
                        1) enc_mode="xorpub" ;;
                        2) enc_mode="native" ;;
                        3) enc_mode="random" ;;
                        *) _err "无效选择"; return 1 ;;
            ...
            if [[ "$reality_profile" == "encryption" ]]; then
                encryption="mlkem768x25519plus.${enc_mode}.0rtt.100-111-1111.75-0-111.50-0-3333.${pubkey}"
                decryption="mlkem768x25519plus.${enc_mode}.600s.100-111-1111.75-0-111.50-0-3333.${privkey}"
            fi
            ...
            if [[ "$reality_profile" == "encryption" ]]; then
                echo -e "  Encryption: ${G}${enc_mode}${NC} ${D}(实验性 VLESS Encryption)${NC}"
            fi
            ...
            _info "生成配置..."
            gen_server_config "$uuid" "$port" "$privkey" "$pubkey" "$sid" "$final_sni" "$encryption" "$decryption"
            ;;
        vless-xhttp)
            # 选择 XHTTP 模式
            echo ""
